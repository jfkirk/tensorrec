...
def rmse_loss(tf_prediction_serial, tf_y_serial, **kwargs):
    """
    ...
    :return:
    """
    return tf.sqrt(tf.reduce_mean(tf.square(tf_y_serial - tf_prediction_serial)))


def separation_loss(tf_prediction_serial, tf_y_serial, **kwargs):
    """
    ...
    tf_positive_mask = tf.greater(tf_y_serial, 0.0)
    tf_negative_mask = tf.less_equal(tf_y_serial, 0.0)
