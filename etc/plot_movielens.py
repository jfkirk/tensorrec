...
n_movies_to_plot = 100

fig, ax = plt.subplots()
ax.scatter(*zip(*movie_positions[:n_movies_to_plot]))

for i, movie_name in enumerate(item_titles[:n_movies_to_plot]):
    ax.annotate(movie_name, movie_positions[i], fontsize='x-small')
fig.show()
